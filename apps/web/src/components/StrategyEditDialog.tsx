...
export function StrategyEditDialog({
  ...
  useEffect(() => {
    if (isOpen && strategy) {
      ...
      setCombine(strategy.configJson?.combine ?? "weighted_sum");
      ...
      setSelected(next);
      // 历史策略可能缺少 entry/exit/risk，缺失时回退默认值
      setEntryThreshold(strategy.configJson?.entry?.value ?? 65);
      setExitThreshold(strategy.configJson?.exit?.value ?? 30);
      setPositionSize(strategy.configJson?.risk?.positionSize ?? 95);
      setStopLoss(strategy.configJson?.risk?.stopLoss ?? 8);
      setTakeProfit(strategy.configJson?.risk?.takeProfit ?? 20);
    }
  }, [isOpen, strategy]);
  ...
  const handleSubmit = () => {
    if (!strategy || !name.trim() || selectedCount === 0 || !weightsValid) return;
    onSubmit({
      id: strategy.id,
      name: name.trim(),
  ...
  return (
    <Dialog isOpen={isOpen} onOpenChange={onOpenChange} purpose="form" width={640} maxHeight="85vh">
      <Layout
        header={<DialogHeader title="编辑策略" subtitle="修改名称、描述、因子组合、入场/出场与风控参数" onOpenChange={onOpenChange} />}
        content={
          <LayoutContent>
            <VStack gap={4}>
              <TextInput
